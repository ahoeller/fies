...
static void visit_type_FaultInfo_params_fields(Visitor *m, FaultInfo ** obj, Error **errp)
{
    Error *err = NULL;
    visit_type_int(m, obj ? &(*obj)->params.mask : NULL, "mask", &err);
    visit_type_int(m, obj ? &(*obj)->params.address : NULL, "address", &err);
    visit_type_int(m, obj ? &(*obj)->params.cf_address : NULL, "cf_address", &err);
    visit_type_int(m, obj ? &(*obj)->params.instruction : NULL, "instruction", &err);
    visit_type_int(m, obj ? &(*obj)->params.set_bit : NULL, "set_bit", &err);

    error_propagate(errp, err);
}

static void visit_type_FaultInfo_fields(Visitor *m, FaultInfo ** obj, Error **errp)
{
    Error *err = NULL;
    if (!error_is_set(errp)) {
        Error **errp = &err; /* from outer scope */
        Error *err = NULL;
        visit_start_struct(m, NULL, "", "params", 0, &err);
        if (!err) {
            if (!obj || *obj) {
                visit_type_FaultInfo_params_fields(m, obj, &err);
                error_propagate(errp, err);
                err = NULL;
            }
            /* Always call end_struct if start_struct succeeded.  */
    ...
    visit_type_str(m, obj ? &(*obj)->component : NULL, "component", &err);
    visit_type_str(m, obj ? &(*obj)->mode : NULL, "mode", &err);
    visit_type_str(m, obj ? &(*obj)->target : NULL, "target", &err);
    visit_type_str(m, obj ? &(*obj)->type : NULL, "type", &err);
    visit_type_str(m, obj ? &(*obj)->duration : NULL, "duration", &err);
    visit_type_str(m, obj ? &(*obj)->interval : NULL, "interval", &err);
    visit_type_int(m, obj ? &(*obj)->id : NULL, "id", &err);
    visit_type_str(m, obj ? &(*obj)->trigger : NULL, "trigger", &err);
    visit_type_str(m, obj ? &(*obj)->timer : NULL, "timer", &err);
    visit_type_int(m, obj ? &(*obj)->is_active : NULL, "is_active", &err);

    error_propagate(errp, err);
}

void visit_type_FaultInfo(Visitor *m, FaultInfo ** obj, const char *name, Error **errp)
{
    if (!error_is_set(errp)) {
        Error *err = NULL;
        visit_start_struct(m, (void **)obj, "FaultInfo", name, sizeof(FaultInfo), &err);
        if (!err) {
            if (!obj || *obj) {
                visit_type_FaultInfo_fields(m, obj, &err);
                error_propagate(errp, err);
                err = NULL;
            }
            /* Always call end_struct if start_struct succeeded.  */
...
void visit_type_FaultInfoList(Visitor *m, FaultInfoList ** obj, const char *name, Error **errp)
{
    GenericList *i, **prev = (GenericList **)obj;
    Error *err = NULL;
    ...
    if (!error_is_set(errp)) {
        ...
        if (!err) {
            for (; (i = visit_next_list(m, prev, &err)) != NULL; prev = &i) {
                FaultInfoList *native_i = (FaultInfoList *)i;
                visit_type_FaultInfo(m, &native_i->value, NULL, &err);
            }
            error_propagate(errp, err);
            err = NULL;
