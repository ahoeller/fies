...
List of all injected faults.

Return a json-object with the following information:

- "component": component name (json-string)
- "mode": mode name (json-string)
- "target": target name (json-string)
- "type": type(json-string)
- "duration": duration (json-string)
- "interval": interval (json-string)
- "timer": timer(json-string)
- "trigger": trigger (json-string)
- "id": id (json-int)
- "params": A json-object containing two integer and one string values:
    - "mask": defines, which bit should be set (json-int)
    - "address": address of the accessed cell (json-int)
    - "cf_address": address of the accessed, coupled cell (json-int)
    - "instruction": address of replaced instruction (json-int)
    - "set_bit": defines if the selected bit by mask should been set or reset (json-int)
- "is_active": fault status (json-int)

Example:

-> { "execute": "query-faults" }
<- {
      "return":{
         "component":"CPU"
...
EQMP

    {
        .name       = "query-faults",
        .args_type  = "",
        .mhandler.cmd_new = qmp_marshal_input_query_faults,
    },
